...
<main class="${empty mainClass ? 'container my-4' : mainClass}">
    ...
    <script>
        ...
        // ==========================================
        // 3. CHAT DROPDOWN (FACEBOOK STYLE)
        // ==========================================
        ...
        // Expose globally so footer.jsp can trigger a refresh
        ...
        function fetchChatContacts(append = false) {
            ...
            fetch("<c:url value='/api/chat/contacts'/>?offset=" + chatContactsOffset + "&limit=" + chatContactsLimit)
                .then(res => res.json())
                .then(data => {
                    const ul = document.getElementById('chat-contact-items');
                ...
                .catch(err => {
                    ...
                    const ul = document.getElementById('chat-contact-items');
                    ul.innerHTML = '<li><span class="dropdown-item-text text-danger small text-center d-block">Lỗi tải dữ liệu. Hãy khởi động lại server.</span></li>';
                    isFetchingChatContacts = false;
                });
        }
